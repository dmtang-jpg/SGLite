...
function Enable-ExeFiles {
    ...
    foreach ($exe in $ExtraExeNames) {
        ...
        if (-not (Test-Path $disabledPath)) { continue }

        try {
            Rename-Item -Path $disabledPath -NewName $exe -Force -ErrorAction Stop
...
function Disable-PluginDirs {
    ...
    foreach ($plugin in $ExtraPluginDirs) {
        ...
        if (Test-Path $disabledPath) {
            Remove-Item $disabledPath -Recurse -Force -ErrorAction SilentlyContinue
        }
...
function Enable-PluginDirs {
    ...
    foreach ($plugin in $ExtraPluginDirs) {
        ...
        try {
            Rename-Item -Path $disabledPath -NewName $plugin -Force -ErrorAction Stop
